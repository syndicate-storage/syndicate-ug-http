...
function extract_path(path) {
    ...
    var idx = path.indexOf("/", start);
    if(idx >= start) {
        // extract session
        s = path.substring(start, idx);
        p = path.substring(idx);
    }
...
module.exports = {
    ...
    get_router: function() {
        ...
        router.get('/setup/session', function(req, res) {
            var session_key = sessions.create_session_key();
            return_data(req, res, {
        ...
        router.post('/setup/gateway', upload.single('cert'), function(req, res) {
            ...
            utils.log_debug(util.format("setting up a gateway - U(%s) / V(%s) / G(%s) for MS(%s) with a session key (%s)", user, volume, gateway, ms_url, session_key));
            async.waterfall([
                function(cb) {
                ...
                function(result, cb) {
                    // register gateway and volume
                    try {
                        var session_key = sessions.create_session(sess, session_key, ms_url, user, volume, gateway, anonymous, get_configuration_path());
                        cb(null, {
                            session_key: session_key
                        });
                    } catch (e) {
        ...
        var r_get = function(req, res) {
            var options = req.query;
            var extracted = extract_path(req.target);
            var session_key = extracted.session;
            var path = extracted.path;
            var session = sessions.get_session(req.sessions, session_key);
            if(session === null) {
                // session not exist
