...
module.exports = {
    ...
    destroy: function(gateway_state) {
        ...
        var fd_keys = gateway_state.fd_map.keys();
        for(var i=0;i<fd_keys.length;i++) {
            utils.log_debug(util.format("closing a missing file handle - %s", fd_keys[i]));
            var stat = gateway_state.fd_map.get(fd_keys[i]);
            if(stat !== null) {
            ...
            gateway_state.fd_map.del(fd_keys[i]);
        }
